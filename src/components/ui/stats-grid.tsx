 ...
 *
 * valueSize: "xl" (기본, 홈 화면) | "2xl" (지원 내역 페이지처럼 강조할 때)
...
export function StatsGrid({ stats, valueSize = "xl", labelSize = "xs", className }: StatsGridProps) {
  return (
    <div
      className={cn(
        ...
        <div key={stat.label} className="text-center py-4">
          <p
            className={cn(
              ...
              valueSize === "2xl" ? "text-2xl" : "text-xl"
            )}
          >
            {stat.value}
          </p>
          <p className={cn("text-primary-foreground/50 mt-2 font-medium", labelSize === "10px" ? "text-[10px]" : "text-xs")}>{stat.label}</p>
        </div>
      ))}
    </div>
  );
